...
import { getOrdersList, getSingleOrder } from "../../redux/ducks/orders/orders";
...
function Orders() {
  ...
  const order = useSelector((data: any) => data.orderStore.order);
  ...
  return (
    <SC.Container>
      ...
      <SC.OrderDetailContainer>
        <SC.DetailHeader>
          <span>{`Detalhe do pedido #${order?.id}`}</span>
        </SC.DetailHeader>
        <SC.InfoWrap>
          <>
            ...
            <SC.Prices>
              ...
              <SC.Row>
                ...
                <SC.DetailTextBold
                  style={{ color: "#0d1136" }}
                >{`R$ ${toCurrency(order?.total_amount)}`}</SC.DetailTextBold>
              </SC.Row>
            </SC.Prices>
          </>
        </SC.InfoWrap>
